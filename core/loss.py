...
def compute_self_loss(logits_neg, y):
    # 注意查看a是不是one-hot
    logits_neg = torch.reshape(logits_neg, (-1, 8))
    # print(y_hat_fl.type())
    y_fl = torch.reshape(y, (-1,))  # Class indices
    y_one_hot = F.one_hot(y_fl, num_classes=8).half()
    prediction_ans_k, top_ans_ind = torch.topk(F.softmax(y_one_hot, dim=-1), k=1, dim=-1, sorted=False)
...
def CEFocalLoss2(y_hat, y, scale_factor=1, num_classes=8, label_smoothing=0., class_weights=None, alpha=0.25, gamma=2.,
                 distillation_loss=True):
    y_hat_fl = torch.reshape(y_hat, (-1, num_classes))
    # print(y_hat_fl.type())
    y_fl = torch.reshape(y, (-1,))  # Class indices
    ...
    # selfloss
    selfloss = compute_self_loss(y_hat_fl, y_fl)
    ...
    if distillation_loss:
        dist_loss_coeff = 0.2
        return loss.mean() * (1 - dist_loss_coeff) + dist_loss_coeff * dist_loss + selfloss * 1
    else:
        return loss.mean() + selfloss * 1
