...
class ABAW3Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        sample = {'image': [], 'index': cur_seq_sample[:, -1].astype(np.int32),
                  'video_id': cur_seq_sample[0, 0].split('/')[0], 'frame': []}
        if self.task == 'mtl':
        ...
        for idx in range(cur_seq_sample.shape[0]):
            # Load image
            cur_img = Image.open(pathlib.Path(self.root_dir, 'cropped-align-images', cur_seq_sample[idx, 0])).convert('RGB')

            if self.transforms is not None:
                cur_img = self.transforms(cur_img)

            sample['image'].append(cur_img)
            # cur_seq_sample[idx, 0] = np.fromstring(cur_seq_sample[idx, 0], dtype=np.int32)
            sample['frame'].append(cur_seq_sample[idx, 0])
        ...
        sample['image'] = torch.stack(sample['image'], dim=0)
        # sample['frame'] = ''.join(sample['frame'], dim=0)
        # sample['frame'] = torch.stack(sample['frame'], dim=0) np.concatenate()
        # sample['frame'] = np.stack(sample['frame'])
        # sample['frame'] = np.array(sample['frame'])
        for ky in ['VA', 'EXPR', 'AU']:
            if ky in sample.keys():
                sample[ky] = np.array(sample[ky])
                sample[ky] = torch.from_numpy(sample[ky])
