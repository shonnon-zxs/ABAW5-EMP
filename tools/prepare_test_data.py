...
if __name__ == '__main__':
    root_video_path = '/data/zhangxuesong/ABAWdataset/videos/batch1/'
    dataset_path = '/data/zhangxuesong/ABAWdataset/'

    for chal in [ 'EXPR',]: #'AU', 'VA',
        with open('/data/zhangxuesong/ABAWdataset/EXPR_test_set_release.txt', 'r') as fd:
            ...
            for vid in tqdm(list_test_video):
                ...
                cropped_aligned = glob.glob('{}/cropped-align-images/{}/*.jpg'.format(dataset_path, vid))
                ...
                for vid_frame in cropped_aligned:
                    list_files[int(vid_frame.split('/')[-1].split('.')[0]) - 1] = vid_frame.replace(
                        '{}/cropped-align-images/'.format(dataset_path), '')
